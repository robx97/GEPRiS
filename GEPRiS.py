...
class GEPRiS:
    ...
    def B12_prediction(self, target_centers, A, kB_gcm2, fC, a=0.033, b=0.009, bp=0.0, c=0.0, alpha=0.0, perturb=False, random_seed=None):
        rho = self.rho
    ...
    def C11_prediction(self, target_centers, A, kB_gcm2, fC, a=0.033, b=0.009, bp=0.0, c=0.0, alpha=0.0,
                      perturb=False, random_seed=None):
        rho = self.rho
        mass_stopping = self.mass_stopping  
        E_vals = self.E_vals  
        electron = self.electron 
    ...
    def nH_prediction(self, target_centers, A, kB_gcm2, fC, a=0.033, b=0.009, bp = 0.0 ,c=0.0, alpha=0.0):
        rho = self.rho
        mass_stopping = self.mass_stopping  
        E_vals = self.E_vals  
        electron = self.electron 
    ...
    def juno_chi2_nl(self, params, E_data, y_data, yerr, b12_data, b12_err, b12_centers,
                     c11_data, c11_err, c11_centers, split=False):
        A, kB_gcm2, fC, N_b12, N_n12, N_c11 = params
        y_pred = self.scint_model(E_data, A, kB_gcm2, fC)
        b12_pred, n12_pred, _ = self.B12_prediction(b12_centers, A, kB_gcm2, fC)
        c11_pred, _ = self.C11_prediction(c11_centers, A, kB_gcm2, fC)

        chi2_gamma = np.sum(((y_pred - y_data) / yerr)**2)
        chi2_b12 = np.sum(((N_b12*b12_pred + N_n12*n12_pred - b12_data) / b12_err)**2)
        chi2_c11 = np.sum(((N_c11*c11_pred - c11_data) / c11_err)**2)

        if split:
            return chi2_gamma, chi2_b12, chi2_c11
        else:
            return chi2_gamma + chi2_b12 + chi2_c11

    def juno_chi2_reso(self, params, E_data, y_data, yerr, 
                       E_rec, sigma, sigma_err,
                       ibd_data, ibd_err, ibd_centers,
                       b12_data, b12_err, b12_centers,
                     c11_data, c11_err, c11_centers, split=False):
        A, kB_gcm2, fC, a, b, bp, c, alpha, N_ibd, N_b12, N_n12, N_c11 = params
        y_pred = self.scint_model(E_data, A, kB_gcm2, fC, alpha)
        ibd_pred = self.nH_prediction(ibd_centers, A, kB_gcm2, fC, a, b, bp, c, alpha)
        b12_pred, n12_pred, _ = self.B12_prediction(b12_centers, A, kB_gcm2, fC, a, b, bp, c, alpha)
        c11_pred, _ = self.C11_prediction( c11_centers, A, kB_gcm2, fC, a, b, bp ,c, alpha)
        resol = self.juno_resolution(E_rec, a, b, c, fit=True)

        chi2_gamma = np.sum(((y_pred - y_data) / yerr)**2)
        chi2_ibd = np.sum(((N_ibd*ibd_pred - ibd_data) / ibd_err)**2)
        chi2_b12 = np.sum(((N_b12*b12_pred + N_n12*n12_pred - b12_data) / b12_err)**2)
        chi2_c11 = np.sum(((N_c11*c11_pred - c11_data) / c11_err)**2)
        chi2_resol = np.sum( ( (resol - sigma) / (sigma_err) )**2)
        chi2_penalty = np.square(alpha - 0) / (0.6e-3)**2

        if split:
            return chi2_gamma, chi2_ibd, chi2_b12, chi2_c11, chi2_resol
        else:
            return chi2_gamma + chi2_ibd + chi2_b12 + chi2_c11 + chi2_resol + chi2_penalty

    def fit(self, E_data, y_data, yerr,
            b12_data, b12_err, b12_centers,
            c11_data, c11_err, c11_centers,
            p0, bounds):
        result = minimize(
            self.juno_chi2_nl, p0,
            args=(E_data, y_data, yerr, b12_data, b12_err, b12_centers,
                  c11_data, c11_err, c11_centers),
            method="L-BFGS-B",
            bounds=bounds
        )
        ...
        H = self.numerical_hessian(
            lambda p: self.juno_chi2_nl(p, E_data, y_data, yerr,
                                        b12_data, b12_err, b12_centers,
                                        c11_data, c11_err, c11_centers),
            popt
        )
        ...
        chi2_gamma, chi2_boron, chi2_carbon = self.juno_chi2_nl(popt, E_data, y_data, yerr,
                                 b12_data, b12_err, b12_centers,
                                 c11_data, c11_err, c11_centers, split=True)
        ndf = [len(y_data) - len(popt), len(b12_data) - len(popt), len(c11_data) - len(popt)]
        return popt, perr, cov, np.array([chi2_gamma, chi2_boron, chi2_carbon]), np.array(ndf)

    def fit_reso(self, E_data, y_data, yerr, E_rec, sigma_data, sigma_err,
                 ibd_data, ibd_err, ibd_centers,
            b12_data, b12_err, b12_centers,
            c11_data, c11_err, c11_centers,
            p0, bounds):
        result = minimize(
            self.juno_chi2_reso, p0,
            args=(E_data, y_data, yerr, E_rec, sigma_data, sigma_err, 
                  ibd_data, ibd_err, ibd_centers,
                  b12_data, b12_err, b12_centers,
                  c11_data, c11_err, c11_centers),
            method="L-BFGS-B",
        ...
        # Covariance and errors
        H = self.numerical_hessian(
            lambda p: self.juno_chi2_reso(p, E_data, y_data, yerr,
                                         E_rec, sigma_data, sigma_err,
                                          ibd_data, ibd_err, ibd_centers,
                                        b12_data, b12_err, b12_centers,
                                        c11_data, c11_err, c11_centers),
            popt
        ...
        chi2_gamma, chi2_ibd, chi2_boron, chi2_carbon, chi2_reso = self.juno_chi2_reso(popt, E_data, y_data, yerr,
                                                                                       E_rec, sigma_data, sigma_err,
                                                                                       ibd_data, ibd_err, ibd_centers,
                                                                                       b12_data, b12_err, b12_centers,
                                                                                       c11_data, c11_err, c11_centers, split=True)
        ndf = [len(y_data) - 4, len(ibd_data) - 5, len(b12_data) - 6, len(c11_data)- 5, len(sigma_data) - 4]
        return popt, perr, cov, np.array([chi2_gamma, chi2_ibd, chi2_boron, chi2_carbon, chi2_reso]), np.array(ndf)
